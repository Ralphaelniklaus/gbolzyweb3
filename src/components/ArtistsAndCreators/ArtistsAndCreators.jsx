...
function ArtistsAndCreators() {
  ...
  const ArtistContract = useContract({addressOrName: ARTIST_CONTRACT_ADDRESS,contractInterface: ARTIST_ABI,signerOrProvider: signer.data || provider});
  ...
  if(!isConnected) return (
    ...
    <ul className="grid grid-cols-1 gap-y-4 gap-x-4 lg:gap-x-6 md:grid-cols-2 lg:grid-cols-3 w-[90%] sm:w-[55%] md:w-[80%] mx-auto mt-8 max-w-6xl pb-8">
        ...
        {artists.length === 0 && <p className='text-slate-400 dark:text-slate-100 font-semibold col-span-2'>No artists have joined.</p>}
      </ul>
      </div>
    </>
  )
